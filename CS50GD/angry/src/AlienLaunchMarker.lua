...
function AlienLaunchMarker:init(world)
    ...
    -- split into three checker
    self.split = false

    -- gives us the aliens x and y
    self.alienx = nil
    self.alieny = nil
end

function AlienLaunchMarker:update(dt)
    if self.split == false and self.launched == true and love.keyboard.isDown('space') then
        self.alienx = self.alien.body:getX()
        self.alieny = self.alien.body:getY()
        
        -- spawn new aliens in the world, passing in user data of player
        self.alien3 = Alien(self.world, 'round', self.alienx, self.alieny, 'Player')
        self.alien2 = Alien(self.world, 'round', self.alienx, self.alieny, 'Player')

        -- apply the difference between current X,Y and base X,Y as launch vector impulse
        self.alien3.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY + 10) * 10)
        self.alien2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY - 10) * 10)
...
function AlienLaunchMarker:render()
    ...
    elseif self.alien2 == nil and self.launched == true then
        self.alien:render()
    else
        self.alien:render()
        self.alien3:render()
