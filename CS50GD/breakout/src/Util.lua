...
function generatePowerup(atlas)
    local x = 64  -- Define x as a local variable
    local y = 192  -- Define y as a local variable
    local quads = love.graphics.newQuad(x, y, 16, 16, atlas:getDimensions())
    return quads
end

function generateKey(atlas)
    local x = 144  -- Define x as a local variable
    local y = 192  -- Define y as a local variable
    local quads = love.graphics.newQuad(x, y, 16, 16, atlas:getDimensions())
    return quads
end
...
function generatelockedbrick(atlas)
    local x = 160  -- Define x as a local variable
    local y = 48  -- Define y as a local variable
    local quads = love.graphics.newQuad(x, y, 32, 16, atlas:getDimensions())
    return quads
end
--[[
