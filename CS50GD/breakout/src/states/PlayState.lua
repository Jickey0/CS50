...
function PlayState:enter(params)
    ...
    self.powerup.multi = true
    self.pstop = false

    self.key = Powerup()
    self.keyactive = false
    self.key.multi = false
...
function PlayState:update(dt)
    ...
    -- only update if we spawned in our two new balls
    if self.pstop then
        self.ball2:update(dt)
        self.ball3:update(dt)
    else
        self.ball2.inPlay = false
        self.ball3.inPlay = false
    end

    -- powerup code for multiball
    if self.powerup:appear(dt) and self.pstop == false then
    ...
    -- powerup code for key
    if self.key:appear(dt) and self.keyactive == false then
    ...
    for j, ball in pairs(self.balls) do
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                ...
                -- if the score is great enough enlarge the paddle
                if self.score > self.expandPoints then
                    if self.paddle.size < 4 then
                        self.paddle.size = self.paddle.size + 1
                        self.expandPoints = self.expandPoints + 2000
                    end
                end
    ...
    if self.ball1.y >= VIRTUAL_HEIGHT and self.ball2.y >= VIRTUAL_HEIGHT and self.ball3.y >= VIRTUAL_HEIGHT or self.ball1.y >= VIRTUAL_HEIGHT and self.pstop == false then
