...
function PlayState:enter(params)
    ...
    self.previousScore = params.score
...
function PlayState:update(dt)
    ...
    if self.player.ending then
        --print("pole unlocked")
        self:spawnEnding()
    end

    if self.scoreAdded == false then
        self.player.score = self.player.score + self.previousScore
...
-- spawns pole when block is unlocked
function PlayState:spawnEnding()
    table.insert(self.level.objects, GameObject {
        texture = 'flag-pole',
        x = self.levelwidth * TILE_SIZE - TILE_SIZE,
        y = 3 * TILE_SIZE,
        width = 16,
        height = 64,
        frame = 2,
