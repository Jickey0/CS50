...
function Dungeon:init(player)
    ...
    self.entityDeathX = 0
    self.entityDeathY = 0
    self.myprojectile = self.currentRoom.myprojectile
end
...
function Dungeon:update(dt)
    ...
    if not self.shifting then    
        ...
        self.potLifted = self.currentRoom.potLifted
        self.entityDeath = self.currentRoom.entityDeath
        self.entityDeathX = self.currentRoom.entityDeathX
        self.entityDeathY = self.currentRoom.entityDeathY
        self.myprojectile = self.currentRoom.myprojectile
    else
...
function Dungeon:addHeart(myX, myY)
    if math.random(2) == 1 then
        local heart = GameObject(
            ...
            myY)
        -- help!
        table.insert(self.currentRoom.objects, heart)
    end
end
